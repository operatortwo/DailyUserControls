...
Class MainWindow
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs)
        ...
        Dim libname As Reflection.AssemblyName = GetType(ImageButton).Assembly.GetName()
        Dim libver As String = libname.Version.ToString
        Dim netver As String = Nothing

        Dim tfa As TargetFrameworkAttribute = [GetType].Assembly.GetCustomAttribute(GetType(TargetFrameworkAttribute))
        If tfa IsNot Nothing Then
            netver = tfa.FrameworkDisplayName
        End If

        If netver Is Nothing Then
            Me.Title = Title & " V " & appver & " - Library V " & libver
        Else
            Me.Title = Title & "  " & netver & "  " & " V " & appver & " - Library V " & libver
        End If
...
#Region "NumericUpDown 1"
    Private Sub NumericUpDown_ValueChanged(sender As Object, e As RoutedPropertyChangedEventArgs(Of Double)) Handles NumericUpDown1.ValueChanged
